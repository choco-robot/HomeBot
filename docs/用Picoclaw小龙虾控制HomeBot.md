...
编辑 Picoclaw 的 `config.json` 配置文件（路径：`~/.picoclaw/config.json` 或 `%USERPROFILE%\.picoclaw\config.json`），在 `tools.mcp.servers` 下添加 homebot 配置：
...
"mcp": {
  ...
  "servers": {
    "homebot": {
      "enabled": true,
      "command": "C:\\Users\\YourName\\.picoclaw\\workspace\\.venv\\Scripts\\python.exe",
      "args": [
        "C:\\Users\\YourName\\.picoclaw\\workspace\\skills\\homebot\\mcp_homebot_server.py"
      ]
    }
  }
}
...
- `args` 是数组格式，包含 MCP 服务器脚本的完整路径
- 环境变量（如 `HOMEBOT_IP`、`ARK_API_KEY`）需在启动 Picoclaw 前设置，或修改 `scripts/robot_config.py` 配置

这样 Picoclaw 启动时会自动加载 HomeBot 的 MCP 服务器，无需手动启动。
...
HomeBot 技能内置了 MCP 服务器，可以通过 Picoclaw 的 MCP 配置实现自动加载，让 LLM 直接调用机器人控制工具。

#### 配置 MCP 服务器

编辑 Picoclaw 的配置文件 `config.json`（通常位于 `~/.picoclaw/config.json`），在 `tools.mcp.servers` 下添加 homebot 配置：
...
- `command`: Python 解释器路径（如果不在 PATH 中，请使用完整路径如 `C:\Python311\python.exe`）
- `args`: MCP 服务器脚本路径（数组格式）
- 环境变量（如 `HOMEBOT_IP`、`ARK_API_KEY`）需在启动 Picoclaw 前设置，或修改 `scripts/robot_config.py` 配置文件

#### 验证 MCP 服务器加载

配置完成后，重启 Picoclaw，在命令行输入：

```
/tools
```

如果看到以下工具列表，说明 MCP 服务器已成功加载：
...
| 自然语言指令 | 实际执行动作 |
|-------------|-------------|
| "前进 10 厘米" | `python scripts/chassis_control.py forward 10` |
| "后退 15 厘米" | `python scripts/chassis_control.py backward 15` |
| "左转 45 度" | `python scripts/chassis_control.py left 45` |
| "右转 90 度" | `python scripts/chassis_control.py right 90` |
| "停车" / "紧急停止" | `python scripts/chassis_control.py stop` |

### 机械臂控制命令示例

| 自然语言指令 | 实际执行动作 |
|-------------|-------------|
| "回原点" / "复位" | `python scripts/arm_control.py home` |
| "设置基座 0 度" | `python scripts/arm_control.py joint base 0` |
| "设置基座 0 度，肩关节 30 度，肘关节 45 度" | `python scripts/arm_control.py joints "base:0,shoulder:30,elbow:45"` |
| "打开夹爪" / "关闭夹爪" | `python scripts/arm_control.py gripper open` / `close` |
| "查看机械臂状态" | `python scripts/arm_control.py status` |

### 姿态动作

使用 `arm_gestures.py` 脚本执行预设动作：
...
# 指定重复次数
python scripts/arm_gestures.py wave --times 5
```

### 查看机器人视角

使用 `what_does_robot_see_workflow.py` 脚本：

```bash
# 捕获图像并自动分析（需要配置 ARK_API_KEY）
