...
- **2026-08-24** - [通用消息总线与 homebot CLI](docs/更新记录.md#2026-08-24---通用消息总线与-homebot-cli)：新增 XPUB-XSUB 通用消息总线（支持用户自定义消息扩展），新增 `homebot` 命令行工具（服务管理/消息调试/Tab 补全/可打包为 exe）
- **2026-07-09** - [LLM 配置与机械臂 WLAN 遥操作](docs/更新记录.md#2026-07-09---llm-配置与机械臂-wlan-遥操作)：支持 `LLM_PROVIDER` 切换 DeepSeek/火山 Ark/通义千问，新增机械臂 WLAN 遥操作应用
- **2026-04-28** - [Choco-Arm 硬件与视觉增强](docs/更新记录.md#2026-04-28---choco-arm-硬件与视觉增强)：新增 SO-101 机械臂 URDF 模型、AtomS3R-M12 摄像头支持、人脸跟踪功能
...
- **SLAM 导航模块**（开发中，见 [`homebot-navi`](../../tree/homebot-navi) 分支）：激光雷达 SLAM 建图与定位、占据栅格地图、A* 全局路径规划、局部避障、里程计/深度估计服务、AprilTag 感知、地图编辑器与仿真环境，支持建图模式与导航模式切换
