...
class CoordinateTransformer:
    ...
    def __init__(self, arm_id: int = 1):
        ...
        # 根据 arm_id 选择对应的偏移配置
        if arm_id == 2:
            self.arm_offset_x = config.mahjong.arm2_offset_x
            self.arm_offset_y = config.mahjong.arm2_offset_y
            self.arm_offset_z = config.mahjong.arm2_offset_z
            self.arm_y_factor = config.mahjong.arm2_y_factor
            arm_name = "arm2"
        else:
            self.arm_offset_x = config.mahjong.arm_offset_x
            self.arm_offset_y = config.mahjong.arm_offset_y
            self.arm_offset_z = config.mahjong.arm_offset_z
            self.arm_y_factor = config.mahjong.arm_y_factor
            arm_name = "arm"
        ...
        # 尝试多个可能的标定文件路径
        possible_paths = [
            config.mahjong.calibration_file,  # 配置中指定的路径
            "calibration_result.json",  # software 目录下的标定结果
            "../calibration_result.json",  # 如果从 src 子目录运行
            str(Path(__file__).parent.parent.parent.parent / "calibration_result.json"),  # 绝对路径
        ]
        ...
        if calibration_file:
            ...
            # 使用配置中的默认矩阵
            self.homography_matrix = np.array(config.mahjong.homography_matrix).reshape(3, 3)
            logger.debug("未找到标定文件，使用配置中的默认值")
        ...
        logger.info(f"CoordinateTransformer 初始化 [{arm_name}]")
    ...
    def table_to_arm(self, table_x: float, table_y: float, 
        ...
        arm_z = table_z - self.arm_offset_z
    ...
    def arm_to_table(self, arm_x: float, arm_y: float, 
        ...
        table_z = arm_z + self.arm_offset_z
