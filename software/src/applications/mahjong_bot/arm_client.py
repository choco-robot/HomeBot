...
class ArmServiceClient:
    ...
    def move_joints(self, joint_angles: Dict[str, float], 
                   speed: int = 500, acc: int = 20, source: str = "mahjong") -> bool:
        """
        移动关节到指定角度
        ...
        Returns:
            是否成功
        """
        # 锁定手腕yaw关节为0，兼容7关节硬件但保持原有6关节运动逻辑
        joints = dict(joint_angles)
        joints["wrist_yaw"] = 0.0
    ...
    def move_to_joint_array(self, joint_array: list, 
                           speed: int = 800, acc: int = 0, source: str = "mahjong") -> bool:
        """
        使用数组格式移动关节
    ...
    def move_lift_platform(self, height: float, speed: int = 1500, acc: int = 0) -> bool:
        """
        控制升降平台
    ...
    def set_torque_enable(self, enable: bool, source: str = "calibration") -> bool:
        """
        设置机械臂扭矩使能状态
        
        Args:
            enable: True=使能扭矩（锁紧）, False=禁用扭矩（可手动拖动）
            source: 控制源标识
        ...
        request = {
            ...
            "source": source,
            "priority": 3  # auto级别，确保优先级足够
        }
        
        response = self._send_request(request)
        if response and response.get("success"):
            status = "使能" if enable else "禁用"
            logger.info(f"机械臂扭矩已{status}")
            return True
        else:
            msg = response.get("message", "未知错误") if response else "无响应"
            logger.error(f"扭矩控制失败: {msg}")
            return False
    
    def set_torque_disable(self, source: str = "calibration") -> bool:
        """
        禁用机械臂扭矩（进入可拖动示教模式）
        
        这是一个便捷方法，等同于 set_torque_enable(False)
        ...
        Returns:
            是否成功
        """
        return self.set_torque_enable(False, source)
...
class SafeArmController:
    ...
    def __init__(self, client: ArmServiceClient):
        ...
        try:
            from configs.config import get_config
            config = get_config()
            self.joint_limits = getattr(config.arm, "joint_limits", {
                "base": (-180, 180),
                "shoulder": (0, 180),
                "elbow": (0, 180),
                "wrist_flex": (-90, 90),
    ...
    def move_joints_safe(self, joint_angles: Dict[str, float], 
        ...
        注：关节角度范围检查已在 Kinematics.solve_for_position 和 ArmService 中完成，
        此处不再重复验证，仅做可选的日志提示。
        ...
        for joint, angle in joint_angles.items():
            if joint in self.joint_limits:
                ...
                if not (min_val <= angle <= max_val):
                    logger.warning(f"关节 {joint} 角度 {angle}° 超出范围 [{min_val}, {max_val}]，但仍将发送指令（由服务端最终限制）")
